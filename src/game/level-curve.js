...
// Difficulty comes from: seconds per object (tight early, 1 s from level 30), how much of the
// board must be eaten (target fraction), how much of it sits in large objects (which need hole
// growth first) and in stacks (slower to harvest), how many objects there are, and how far
// apart they are (table size). Early levels ramp these quickly so 10 -> 25 is a visible step.
export function levelParams(n, mode = 'normal') {
  ...
  const count = Math.round(lerp(45, 1000, Math.pow(t, 1.4)));
  const early = Math.min(1, (n - 1) / 29); // 0 at level 1, 1 from level 30
  ...
  const sM = lerp(0.25, 0.4, Math.sqrt(t));
  ...
  return {
    ...
    targetFraction: lerp(0.65, 0.9, Math.sqrt(t)),
    secondsPerObject: lerp(0.6, 1.0, early),
