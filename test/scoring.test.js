...
describe('scoring rules', () => {
  ...
  it('default milestones at 25/50/75/100% of target', () => {
    expect(T).toEqual([25, 50, 75, 100]);
    expect(milestonesReached(0, T)).toBe(0);
    expect(milestonesReached(24, T)).toBe(0);
    expect(milestonesReached(25, T)).toBe(1);
    expect(milestonesReached(74, T)).toBe(2);
    expect(milestonesReached(75, T)).toBe(3);
    expect(milestonesReached(150, T)).toBe(4);
  });
  it('milestone progress runs 0..1 between thresholds and is full at the end', () => {
    expect(milestoneProgress(0, T)).toBe(0);
    expect(milestoneProgress(10, T)).toBeCloseTo(0.4);
    expect(milestoneProgress(25, T)).toBe(0);
    expect(milestoneProgress(30, T)).toBeCloseTo(0.2);
  ...
  it('reachable thresholds cap each milestone at 70% of points collectable at that size', () => {
    ...
    // Steps 0..3 only reach the 10 small points -> 7 each: the hole opens fully at 7.
    expect(t).toEqual([7, 7, 7, 7]);
...
describe('session', () => {
  ...
  it('losing fruit off the edge lowers thresholds so growth stays reachable', () => {
    ...
    const accessible = [10, 10, 10, 10, 10];
    const sess = createSession({ target: 100, seconds: 30, fruitCount: 10, thresholds: [7, 7, 7, 7], accessible });
    ...
    expect(sess.state.thresholds).toEqual([4, 4, 4, 4]);
    expect(sess.holeStep).toBe(4);
  });

  it('a bomb after the last milestone shrinks the hole only temporarily', () => {
    const sess = createSession({ target: 10, seconds: 60, fruitCount: 5, thresholds: [0, 0, 0, 0] });
    expect(sess.holeStep).toBe(4);
    sess.swallow(bomb());
    sess.swallow(bomb());
    expect(sess.holeStep).toBe(2);
